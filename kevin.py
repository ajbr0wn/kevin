...
class MySQLEnumerator:
    ...
    def _get_basic_info(self, target: str, port: int, finding: MySQLFinding):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                protocol_match = re.search(r'Protocol version: ([^\n]+)', output)
                if protocol_match:
                    finding.protocol_version = protocol_match.group(1).strip()
                
                # Check for SSL
                if 'SSL' in output or 'TLS' in output:
                    finding.ssl_enabled = True
                
                print(f"[+] MySQL version: {finding.version or 'Unknown'}")
                if finding.ssl_enabled:
                    print(f"[+] SSL/TLS appears to be enabled")
                
        except subprocess.TimeoutExpired:
            print(f"[!] mysql-info script timed out")
        except Exception as e:
            print(f"[!] Error running mysql-info: {e}")
    
    def _analyze_security_config(self, target: str, port: int, finding: MySQLFinding):
        """Analyze MySQL security configuration"""
        print(f"[*] Phase 2: Security configuration analysis...")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if 'root account has empty password' in output.lower():
                    finding.empty_password_accounts.append('root')
                    finding.security_issues.append("ROOT account has empty password - critical security risk!")
                    print(f"[!] CRITICAL: Root account has empty password!")
                
                # Look for other empty password accounts
                empty_accounts = re.findall(r'account (\w+) has empty password', output.lower())
                for account in empty_accounts:
                    if account not in finding.empty_password_accounts:
                        finding.empty_password_accounts.append(account)
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if 'VULNERABLE' in output.upper():
                    ...
                    for vuln in vulns:
                        finding.security_issues.append(f"Audit finding: {vuln}")
                        print(f"[!] {vuln}")
                
                # Check for anonymous access
                if 'anonymous' in output.lower() and 'access' in output.lower():
                    finding.anonymous_access = True
                    finding.security_issues.append("Anonymous access appears to be enabled")
                    print(f"[!] Anonymous access may be enabled")
                
        except Exception as e:
            print(f"[!] Error running mysql-audit: {e}")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                for var in self.important_variables:
                    ...
                    if match:
                        finding.variables[var] = match.group(1).strip()
                
                # Analyze security-relevant variables
                if 'local_infile' in finding.variables:
                    if finding.variables['local_infile'].upper() == 'ON':
                        finding.security_issues.append("local_infile is enabled - potential for file read attacks")
                        print(f"[!] local_infile is enabled")
                
                if 'secure_file_priv' in finding.variables:
                    if not finding.variables['secure_file_priv'] or finding.variables['secure_file_priv'] == 'NULL':
                        finding.security_issues.append("secure_file_priv not properly configured - file operations unrestricted")
                        print(f"[!] secure_file_priv not properly configured")
                
        except Exception as e:
            print(f"[!] Error getting MySQL variables: {e}")
    
    def _enumerate_databases_users(self, target: str, port: int, finding: MySQLFinding):
        """Enumerate databases and users (if accessible)"""
        print(f"[*] Phase 3: Database and user enumeration...")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if finding.databases:
                    print(f"[+] Found {len(finding.databases)} databases: {', '.join(finding.databases[:5])}")
                    if len(finding.databases) > 5:
                        print(f"    ... and {len(finding.databases) - 5} more")
                
        except Exception as e:
            print(f"[!] Error enumerating databases: {e}")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                # Parse usernames
                user_matches = re.findall(r'User: ([^\n]+)', output)
                for user in user_matches:
                    user_clean = user.strip()
                    if user_clean and user_clean not in finding.users:
                        finding.users.append(user_clean)
                
                if finding.users:
                    print(f"[+] Found {len(finding.users)} users: {', '.join(finding.users[:5])}")
                    if len(finding.users) > 5:
                        print(f"    ... and {len(finding.users) - 5} more")
                
        except Exception as e:
            print(f"[!] Error enumerating users: {e}")
    
    def _assess_security_issues(self, finding: MySQLFinding):
        """Assess overall security posture and add recommendations"""
        ...
        if finding.version:
            ...
            if any(old_ver in version_lower for old_ver in ['5.0', '5.1', '4.']):
                finding.security_issues.append(f"Very old MySQL version detected: {finding.version}")
                print(f"[!] Very old MySQL version: {finding.version}")
            
            # Check for versions with known issues
            if '5.5' in version_lower:
                finding.security_issues.append("MySQL 5.5 has known security issues - upgrade recommended")
            elif '5.6' in version_lower:
                finding.security_issues.append("MySQL 5.6 has some known security issues - check for patches")
        
        # SSL assessment
        if not finding.ssl_enabled:
            finding.security_issues.append("SSL/TLS not detected - data transmitted in cleartext")
            print(f"[!] SSL/TLS not detected")
        
        # Overall security assessment
        critical_issues = len([issue for issue in finding.security_issues 
                             if any(keyword in issue.lower() for keyword in ['critical', 'empty password', 'root'])])
    ...
    def run_comprehensive_scan(self, target: str, port: int = 3306) -> subprocess.Popen:
        """Run all MySQL scripts in a single nmap command for efficiency"""
        
        # Create comprehensive script list
        script_args = ','.join(self.mysql_scripts)
        
        cmd = [
            'nmap', '--script', script_args,
            '-p', str(port),
            '--script-args', 'mysql.timeout=10',
            '-v', target
        ]
        
        print(f"[*] Running comprehensive MySQL scan: {' '.join(cmd[:6])}...")
        return subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    
    def parse_comprehensive_output(self, output: str, target: str, port: int) -> MySQLFinding:
        """Parse output from comprehensive MySQL scan"""
        finding = MySQLFinding(
            target=target,
            port=port,
            timestamp=datetime.now().isoformat()
        ...
        # Parse all the information from the comprehensive output
        lines = output.split('\n')
        current_script = None
        ...
        for line in lines:
            ...
            # Detect which script output we're looking at
            if '| mysql-' in line:
                current_script = line.split('| mysql-')[1].split(':')[0]
            
            # Parse based on current script context
            if current_script == 'info':
                if 'Version:' in line:
                    finding.version = line.split('Version:')[1].strip()
                elif 'Server version:' in line:
                    finding.server_version = line.split('Server version:')[1].strip()
                elif 'Protocol version:' in line:
                    finding.protocol_version = line.split('Protocol version:')[1].strip()
            
            elif current_script == 'empty-password':
                if 'root account has empty password' in line.lower():
                    finding.empty_password_accounts.append('root')
                    finding.security_issues.append("ROOT account has empty password!")
            
            elif current_script == 'databases':
                if line.startswith('|') and 'Database:' in line:
                    db_name = line.split('Database:')[1].strip()
                    if db_name not in finding.databases:
                        finding.databases.append(db_name)
            
            elif current_script == 'users':
                if line.startswith('|') and 'User:' in line:
                    user_name = line.split('User:')[1].strip()
                    if user_name not in finding.users:
                        finding.users.append(user_name)
